...
public class TransactionTestExecutor extends TestExecutor {
  ...
  public void testNoneRepeatableRead() throws Exception {
    CompletableFuture<Void> future;
    client1.executeStatement("begin transaction");
    client2.executeStatement("begin transaction");
    try {
      ...
      Thread.sleep(1000);
      if (CommonConfig.TEST_SERIALIZE) {
        queryAndCheckOneColumn(
            client1, "select count from tx where id = 2;", Collections.singletonList("2"));
        LOGGER.info("Select value of table1(id=2) is 2. None repeatable read test success!");
      } else {
        queryAndCheckOneColumn(
            client1, "select count from tx where id = 2;", Collections.singletonList("100"));
        LOGGER.info("Select value of table1(id=2) is 100. None repeatable read test success!");
      }
    } finally {
      client1.executeStatement("commit;");
    }
